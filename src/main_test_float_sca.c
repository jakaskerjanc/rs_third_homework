...
void save_weights_biases(char* file_name) {
    ...
    fwrite(weight1, sizeof(float), HIDDEN_NODES * INPUT_NODES, file);
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        printf("Usage: %s <hidden_nodes> <number_of_epochs> <saved_weight_file>\n", argv[0]);
        return 1;
    }

    HIDDEN_NODES = atoi(argv[1]);
    NUMBER_OF_EPOCHS = atoi(argv[2]);
    char *saved_weight_file = argv[3];

    // load weights and biases
    allocate_memory();
    load_weights_biases(saved_weight_file);
    ...
    printf("Training Accuracy: %f\n", (float) correct_outcomes / NUM_TRAINING_IMAGES);
